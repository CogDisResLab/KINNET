...
render_reduced_kinased_graph <-
  function(analysis_result, title, render = FALSE) {
    ...
    mapped_net_arcs <- net_arcs %>%
      tibble::as_tibble() %>%
      mutate(from = mapped_kinases[.data$from],
    ...
    bnlearn::arcs(bn, check.cycles = FALSE, check.illegal = FALSE) <-
      mapped_net_arcs
    ...
    gr <-
      Rgraphviz::layoutGraph(network_graph, attrs = list(graph = list(rankdir = "TB")))
