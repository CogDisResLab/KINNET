...
annotation <-
  read_csv(
    "data-raw/data/RegPhos_Annotation_human.csv",
    col_types = cols(.default = col_character())
  ) |>
  mutate(
    Kinase = str_to_upper(Kinase),
...
uka_map <-
  read_csv("data-raw/data/uka_pep2kinase_STK.csv",
           col_types = cols(.default = col_character())) |>
...
krsa_map <-
  read_csv(
    "data-raw/data/KRSA_Mapping_STK_Separated.csv",
    col_types = cols(.default = col_character())
  )
...
stk_annotation <- bind_rows(
  ...
  unique() |>
  write_csv("data-raw/data/STK_Annotation.csv")
